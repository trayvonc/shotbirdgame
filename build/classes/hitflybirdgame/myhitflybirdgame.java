...
public class myhitflybirdgame extends JFrame implements ActionListener, MyListener, KeyListener {
    ...
    public myhitflybirdgame() {
        ...
        jm4=new JMenu("人称选择");
        jmi10 = new JMenuItem("第一人称");
        jmi11 = new JMenuItem("第三人称");
        jmi10.addActionListener(this);
        jmi10.setActionCommand("first");
        jmi11.addActionListener(this);
        jmi11.setActionCommand("third");
        //加入组件
        jm1.add(jmi1);
        jm1.add(jmi4);
        jm1.add(jmi3);
...
//抗锯齿
        ...
        drawBlood(g2d);
        if(myhitflybirdgame.changebullet<=50){
            g.setFont(new Font("Lucida Console", Font.PLAIN, 18));
          g.drawString("Reloading...", 0, 28);  
        }
        //音乐提示
        if(myhitflybirdgame.playMusic){
        ...
        //从ss中取出每一颗子弹并画出
       for(int i=0;i<this.gun2.ss.size();i++)
       {

          Shot myShot=gun2.ss.get(i);
        //画出子弹，画出每一颗子弹
        if(myShot!=null&&myShot.isLive==true)
        {
            g.setColor(Color.black);
            g.fill3DRect(myShot.x, myShot.y, 5, 5, false);
        }
        if(myShot.isLive==false)
        {
            //从ss中删除掉该子弹
            gun2.ss.remove(myShot);
        }
       }
        //System.out.println(this.gun2.ss.size());
        //画枪
        if (myhitflybirdgame.drawpic != 1) {
            this.drawGun(gun.x, gun.y, g2d);
            if(Recorder.mode==0)
            Gun2.drawgun(gun.x, gun.y, g2d);
        }
        //暂停
        if (myhitflybirdgame.drawpic == 1) {
            g.drawImage(image26, 360, 300, this);
        }
        //g.drawString(myhitflybirdgame.changebullet+"", 0, 30);
...
//        g.setStroke(new BasicStroke(2, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL));
//        g.setPaint(new GradientPaint(180, 165, Color.CYAN, 295, 165, Color.MAGENTA, true));
        g.fillRect(0, 5, Recorder.getBdsNum()*3, 5);
        //g.setColor(Color.black);
    }
    ...
    public void run() {
        ...
        while (true) {
            ...
            if(myhitflybirdgame.changebullet>=50&&myhitflybirdgame.changebullet<51){
            //换子弹函数
            int temp = Recorder.getBullet();
            if (Recorder.getAllbullet()+Recorder.getBullet() >= 10) {
                Recorder.setBullet(10);
...
boolean kill=false;
    ...
    public void mousePressed(MouseEvent e) {
        if (Recorder.getBullet() == 0) {
            if(Recorder.getBdsNum()>0&&Recorder.getAllbullet()!=0){
                //System.out.println(Recorder.getBdsNum());
            try {
                AePlayWave as = new AePlayWave("src/music/nobullet.wav");
                as.start();
            } catch (Exception e1) {
                e1.printStackTrace();
            }
            ...
            //System.out.println("请按R换子弹");
        }else if (Recorder.mode==0) {
         if(!kaihuo)
         {
             //全自动
         reduce=new Thread(gun2);
         kill=true;
         reduce.start();
         }else{
            gun2.shotEnemy();
...
//            }
    ...
    public void hitEnemy(ArrayList<Bird> bds)
    {
         //判断是否击中
            for(int i=0;i<gun2.ss.size();i++)
            {
                //取出子弹
                Shot myShot=gun2.ss.get(i);
                //判断子弹是否有效
                if(myShot.isLive)
                {
                    ...
                    for(int j=0;j<bds.size();j++)
                    {
                        //取出坦克
                        Bird bd=bds.get(j);
                        if(bd.isLive||bd.tomedal)
                        {
                            this.hitBird(myShot, bd);
                        }
                    }
                }
            }
    ...
    //写一个函数专门判断子弹是否击中坦克
    public void hitBird(Shot s,Bird bd)
    { 

                    //击中
                    //子弹死亡
        if (s.x >= (bd.x-15)  && s.x <= (bd.x + 33) && s.y >= (bd.y-15)  && s.y <= (bd.y + 33) && (bd.isLive||bd.tomedal)) {
